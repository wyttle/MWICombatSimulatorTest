...
2. **Start Simulation** compares the draft and original on paired seeds. **Auto-tune thresholds** explores the ticked numeric conditions. Leave either bound empty for an engine-derived domain; explicit bounds remain hard constraints. The step remains the final grid resolution. Automatic domains use tiered monsters, permitted spawn groups, living-unit aggregation, player resource formulas and consumable envelopes. They include the engine's empty-set percentage sentinel and one grid step outside the physical envelope to cover constant-condition behavior. Skill damage/cast/cooldown/DoT formulas supply initial search scales, not a claim about actual DPS or an optimal HP threshold. *Search algorithm* (advanced settings) chooses how the ticked thresholds are explored:
   - **Bayesian optimization (joint)**, the default, fits one Gaussian-process model (Matérn 5/2, per-threshold length scales, noise fitted by MAP around the observed paired variance) to every evaluation so far and proposes whole threshold vectors by expected improvement, several per batch so the worker queue stays full. It can move two or more thresholds at once, so it finds combinations that only help when changed together. Inputs are warped piecewise-linearly through the engine-derived breakpoints so that a domain such as -100–129,100 still resolves small values. It starts with a Latin-hypercube design, never re-simulates a configuration, and picks the evaluated configuration with the highest posterior mean rather than the highest raw sample mean, which reduces winner's-curse optimism. It has no convergence point: it stops at the budget, or early if no unmeasured candidate remains. The suggested budget is 12 evaluations per threshold plus 12; more budget raises the chance of finding coupled combinations.
   - **Coordinate search (one at a time)** is the previous method: the coarse sweep retains up to three separated peak regions for refinement and revisits interacting coordinates in a second sweep; narrow unsampled peaks and coupled improvements can still be missed.

   Benchmark at equal evaluation budgets (Node worker threads running the browser worker code). Synthetic noisy objectives, 20 repetitions, 16 seeds: on a two-threshold coupled optimum Bayesian optimization found the optimum in 20/20 runs at 40, 80 and 160 evaluations, coordinate search in 1, 8 and 9 of 20; on an 11-threshold sparse case the mean regret was 2.16/1.40/0.84 against 3.74/2.64/2.64; on an additive four-threshold case (coordinate search's best case) 3.09/1.88/1.77 against 4.50/2.90/1.96. On the real 11-threshold fixture (auto domains, 80 evaluations, 16 seeds × 40 attempts, 64 holdout seeds × 200 attempts, 4 repetitions) the holdout ΔDPS was +0.45, +1.23, −0.27, +0.90 for Bayesian optimization and −0.51, +0.38, −0.17, +0.55 for coordinate search, at the same wall time (≈275 s on 30 threads). Real gains on that fixture are within noise for both methods; the real benchmark shows no regression, not a proven improvement. Model fitting adds a few seconds per run at 160 evaluations.
...
**Import report** reads an exported report JSON back into the optimizer on any browser: it is added to the history with a new ID (fields missing from older exports are filled in) and shown as the current report, so its candidates can be applied. Pasting a report JSON into the main **Import** dialog does the same and additionally loads the report's best team (first validated candidate, else the search's best configuration, else the draft) into the roster. Pasting anything that is neither a team export nor a report now raises an error instead of corrupting the roster.

Bayesian runs change several thresholds per evaluation, so measured points rarely share all other thresholds and cannot be joined into one curve. Their chart instead shows the model slice through the chosen configuration: the posterior mean ΔDPS as each threshold varies alone, with a shaded mean ± 1.96 sd band. The band is model uncertainty, not a measurement; only overlaid points (evaluations equal to the chosen vector in every other threshold) are measured. The threshold axis defaults to a signed log scale when the range is wide. Alternatives show the model estimate next to the measured search mean. Resuming after a reload replays the same deterministic proposals, as with coordinate search; the proposal random stream is derived from the exploration seeds.

Exploration seeds are a fixed set derived from a constant, so a configuration simulated for the same team, dungeon, difficulty, buffs, shrine levels and exploration attempt count has the same per-seed samples in every run. Those samples are cached in IndexedDB across runs (keyed by configuration and seed, 60,000 entries, oldest evicted) and, with **Reuse earlier evaluations** ticked, read back instead of re-simulated. The typical loop of *run → apply → run again* therefore starts from the previous run's measurements: the baseline, the initial design and any repeated proposal cost nothing. Paired differences are computed from raw per-seed samples, so a reused sample is identical to a fresh one. Final holdout validation always draws fresh random seeds disjoint from the exploration set; repeated exploration on one seed set can slightly favor that set, which the holdout result does not share. The Bayesian input warp deliberately excludes the current threshold so that applying a result does not shift the initial design and defeat the cache.

Bayesian results also list each threshold's sensitivity, the inverse squared length scale normalized to the most influential threshold. It is a model estimate: a threshold rated near zero had no visible effect on ΔDPS within the explored domain, and unticking it in the next run spends the budget on the others. With eleven thresholds across several members this is usually the fastest way to shrink the problem; the ranking is only meaningful after enough evaluations for the model to fit.

Bayesian exploration races candidates: with at least four seeds, each proposal is first simulated on half of the exploration seeds; the model then treats those points with proportionally larger noise and completes only candidates whose posterior is within about one standard deviation of the incumbent (the best fully evaluated configuration), plus the batch leader within two. The budget counts seed-runs in evaluation equivalents, so a half-evaluated candidate costs 0.5 and a budget of 80 measures noticeably more configurations than 80 full evaluations. Only fully evaluated configurations can win or appear as alternatives; partially evaluated ones remain in the history with their smaller sample counts marked. At equal simulation cost on the synthetic suite, half-seed racing matched no racing on the coupled, additive and multimodal cases and improved the 11-threshold sparse case at 160 evaluations (regret 0.50 against 0.84); a quarter-seed first stage was measurably worse and is not used. On the real fixture (same protocol as above, 2 repetitions) racing gave holdout ΔDPS −0.26 and +0.17 in about 300 s per run against about 275 s without racing; on that fixture every variant so far is within noise.
