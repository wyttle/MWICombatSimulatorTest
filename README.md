...
Memory, not CPU, is what caps the worker count. One simulation worker peaks around 160–175 MB of heap regardless of how many dungeons it runs, because V8 lets garbage pile up that far before collecting — the live data is only 11–14 MB, and the same workload finishes in the same wall time under a 64 MB heap cap. Browsers expose no such cap for workers, so the peak simply multiplies by the number of workers: a 16-core machine picking its core count would run 15 workers and ask for 3 GB, which crashes the tab out of memory on long dungeon runs. Both the main page and the optimizer therefore clamp the slider to a memory budget (`src/workerBudget.js`, 1.6 GB / 200 MB per worker = 8), and the optimizer prints the projected footprint under the slider. Lower the worker count before lowering the dungeon count if a run dies.
